...
function parsePlayerData(buf, teamKey) {
    ...
    // Debug: Log full buffer for Team A to find the data
    if (teamKey === 'teamA') {
        console.log(`Debug Team A Payload (Len ${buf.length}): ${Array.from(buf).map(b => b.toString(16).padStart(2, '0')).join(' ')}`);
    }
...
function parsePenaltyData(buf, teamKey) {
    ...
    // Or maybe the table means:
    // Item: Penalty.
    // Sub-items:
    // 1. Player Number (3)
    // 2. Penalty Time (2) ==> Penalty 1
    // 3. Player Number (3)
    // 4. Penalty Time (2) ==> Penalty 2
    // ...
    // If Length is 12, maybe 2 penalties = 10 bytes. The other 2 bytes?
    // Maybe the doc screenshot is cut off or I'm misreading Length=12.
...
function formatShotClock(sec, tenth) {
    ...
    if (sec > 5) {
        return (tenth >= 5 ? sec + 1 : sec).toString();
    }
    return `${sec}.${tenth}`;
}
